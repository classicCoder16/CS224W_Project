...
def get_board_users_unweighted(G):
	...
	f = open("user_board_small_train.txt", "w")
	for n in G.Nodes():
		n = n.GetId()
		if n > 31097108: G.DelNode(n)
	nodes = sorted([n.GetId() for n in G.Nodes()], reverse=True)
...
def get_board_pins_unweighted(G):
	print "reading boards"
	f = open("pin_board_small_train.txt", "w")
	for n in G.Nodes():
		n = n.GetId()
		if n <= 18630353: G.DelNode(n)
	nodes = sorted([n.GetId() for n in G.Nodes()])
...
def get_board_unweighted(G):
	print "reading boards"
	f = open("board_small_train.txt", "w")
	nodes = sorted([n.GetId() for n in G.Nodes()])
	count = 0
	for n in nodes:
...
def main():
	G = snap.LoadEdgeList(snap.PUNGraph, "smallest_train_graph.txt", 0, 1)
	G.DelNode(0)
