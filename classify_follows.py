...
def validate_train(train_examples, train_labels, graph, board_ids):
	...
	for src_id, dst_id in train_examples:
		...
		if not graph.IsEdge(src_id, dst_id) and train_labels[i] == 1:
			print 'Conflict!'
		if not ((isBoard(src_id, board_ids) and isUser(dst_id, board_ids)) or \
			(isBoard(dst_id, board_ids) and isUser(src_id, board_ids))):
			print 'Conflict!'
		i += 1
...
def isUser(n, board_node_ids):
	return n < board_node_ids[0]
...
def main(input_train, input_test, output_root):
	...
	train_examples, train_labels = get_foll_tr_ex(max_scc, 5000, 5000, board_ids, train_graph_obj.attributes)
	...
	test_examples, test_labels = get_foll_tst_ex(max_scc, test_pgraph, \
	...
	try:
		np.save('train_' + output_root + '_fol_features', all_train_features)
		np.save('test_' + output_root + '_fol_features', all_test_features)
		np.save('train_' + output_root + '_fol_examples', zip(train_examples, train_labels))
		np.save('test_' + output_root + '_fol_examples', zip(test_examples, test_labels))
